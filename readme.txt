=== Quick Box - Popup Notification Box===
Contributors: f1logic
Donate link: http://xyzscripts.com/donate/
Tags: quick popup, quick box, quick popup box, quick box popup, on click popup, on click quick popup, onclick popup, custom message popup, popup message on pages, popup message,popup, popup box
Requires at least: 2.8
Tested up to: 4.7.5
Stable tag: 1.2.1
License: GPLv2 or later

Create a javascript based, light-weight and non-annoying onclick quick  popup box in your blog.

== Description ==

A quicklook into Quick Box - Popup Notification Box

    ★ Create a light weight quick onclick popup box
    ★ Full control on popup content
    ★ Standard WordPress content editor
    ★ Display based on browsed number of pages
    ★ Css settings like color, z-index, border etc
    ★ Shortcode for displaying popup in specific pages
    ★ Display specificaly in pages/posts/home page

This plugin allows you to create a Quick Box - Popup Notification Box window with custom content in your site. You can customize the popup display by configuring various settings such as display logic settings and style settings(z-index, color, border etc). The plugin supports automatic and manual (shortcode) display. You can use the plugin to display any type of contents such as special promotions, subscription forms, social media icons, feedback forms, video presentations and much more.

= Want more features ? =

Check out the [premium version](http://xyzscripts.com/wordpress-plugins/xyz-wp-popup/ "XYZ WP Popup") of this plugin.

= About =

Quick Box - Popup Notification Box is developed and maintained by [XYZScripts](http://xyzscripts.com/ "xyzscripts.com"). For any support, you may [contact us](http://xyzscripts.com/support/ "XYZScripts Support").

★ [Quick Box - Popup Notification Box User Guide](http://help.xyzscripts.com/docs/quick-box-popup/ "Quick Box - Popup Notification Box User Guide")
★ [Quick Box - Popup Notification Box FAQ](http://help.xyzscripts.com/docs/quick-box-popup/faq/ "Quick Box - Popup Notification Box Popup FAQ")

== Installation ==

★ [Quick Box - Popup Notification Box User Guide](http://help.xyzscripts.com/docs/quick-box-popup/ "Quick Box - Popup Notification Box User Guide")
★ [Quick Box - Popup Notification Box FAQ](http://help.xyzscripts.com/docs/quick-box-popup/faq/ "Quick Box - Popup Notification Box FAQ")

1. Extract `quick-box-popup.zip` to your `/wp-content/plugins/` directory.
2. In the admin panel under plugins activate Quick Box - Popup Notification Box.
3. You can configure the settings from XYZ Quick Box menu.
4. It should now be completely set up and functional.

If you need any further help, you may contact our [support desk](http://xyzscripts.com/support/ "XYZScripts Support").

== Frequently Asked Questions ==

★ [Quick Box - Popup Notification Box User Guide](http://help.xyzscripts.com/docs/quick-box-popup/ "Quick Box - Popup Notification Box User Guide")
★ [Quick Box - Popup Notification Box FAQ](http://help.xyzscripts.com/docs/quick-box-popup/faq/ "Quick Box - Popup Notification Box FAQ")

= 1. What is a "Quick Box - Popup Notification Box" ? =

A Quick Box - Popup Notification Box is used to display any content inside a popup within a webpage. It is normally used to show subscription forms, contact forms, promotions, etc.

= 2. The Quick Box - Popup Notification Box plugin is not working properly =

Please check the wordpress version you are using. Make sure it meets the minimum version recommended by us. Make sure all files of the `Quick Box - Popup Notification Box` plugin are uploaded to the folder `wp-content/plugins/`

= 3. Can I use this plugin with other popup plugins ? =

You can use it with any other popup related plugins from XYZScripts. However it may interfere with popup plugins from other vendors.

= 4. Can I set repeat interval in minutes ? =

Yes, you can set repeat interval in hours as well as minutes.

= 5. What kind of contents can be placed in a popup? =

You may  use the popup to display any type of contents such as special promotions, subscription forms, feedback forms, video presentations and much more.

= 6. Can I display popup in specific pages only ? =

Yes, you can do this by using the shortcode provided from settings page.

= 7. Does it work with caching plugins ? =

You can turn on compatibility with cache plugins from settings page. Please  note that since this method uses ajax, the popup will be a rendered a bit slower than in normal mode.

= 8. Can I create multiple popup boxes for different pages ? =

You need to purchase the premium version for having this feature.

= 9. Where can i get the premium version ? =

The premium version can be purchased from our site [xyzscripts.com](http://xyzscripts.com/wordpress-plugins/xyz-wp-popup/ "XYZ WP Popup").

More questions ? [Drop a mail](http://xyzscripts.com/support/ "XYZScripts Support") and we shall get back to you with the answers.


== Screenshots ==

1. Sample Quick Box - Popup Notification Box which has a feedback form.

== Changelog ==

= 1.2.1=
* Nonces added
* Data validations updated
* Direct access prevented in all plugin files
* the_editor()  changed to wp_editor()
* get_currentuserinfo() changed to wp_get_current_user()
* 'htmledit_pre' hook changed to 'format_for_editor' for fixing Tinymce formatting issue
* A few other bug fixes

= 1.2 =
* Responsive quick box
* Fixed a few bugs

= 1.1.1 =
* Fix for enable popup issue.

= 1.1 =
* Display specifically in pages/posts/home page.
* Enable/disable premium version ads.
* Option to enable/disable popup.
* Compatibility with cache plugins.
* Network installation support.

= 1.0.1 =
* Fix for shortcode issue.

= 1.0 =
* First official launch.

== Upgrade Notice ==

= 1.0.1 =
If you have had issues with shortcode, you must do this upgrade.


== More Information ==

★ [Quick Box - Popup Notification Box User Guide](http://help.xyzscripts.com/docs/quick-box-popup/ "Quick Box - Popup Notification Box User Guide")
★ [Quick Box - Popup Notification Box FAQ](http://help.xyzscripts.com/docs/quick-box-popup/faq/ "Quick Box - Popup Notification Box FAQ")

= Troubleshooting =

Please read the FAQ first if you are having problems.

= Requirements =

Server

    WordPress 2.8+
    PHP 5+ (Recommended)

Client

    IE 7+, FireFox 2+, Chrome, Safari 3+, Opera 8+


= Feedback =

We would like to receive your feedback and suggestions for the betterment of this plugin. You may submit them at our [support desk](http://xyzscripts.com/support/ "XYZScripts Support").
